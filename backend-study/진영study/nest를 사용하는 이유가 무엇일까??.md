...
2. 개발 폐쇄 원칙: 확장에 대해서는 열려있되 수정에 대해서는 닫혀있어야 한다. => 내 식대로 설명하자면 그떄그때 필요한 로직이 있다면 로직을 확장시켜서 만들면 된다. 하지만 만들어진 로직을 수정하는건 지양해야한다. 왜냐하면 그 로직이 사용된 서비스에서 그 로직이 수정됨에 따라 어떤 영향을 받을지 모르기 때문이다. 즉 유지보수를 위해서는 수정은 별로 안좋다, 하지만 새로운 비지니스로직을 확장시키는건 문제 없다.
